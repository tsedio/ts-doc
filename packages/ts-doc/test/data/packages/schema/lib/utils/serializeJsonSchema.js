...
function serializeInherited(obj, target, options = {}) {
  const stores = Array.from(getInheritedStores_1.getInheritedStores(target).entries()).filter(
    ([model]) => core_1.classOf(model) !== core_1.classOf(target)
  );
  if (stores.length) {
